  ...
  <main class="inline">
    <section>
      ...
      <ul class="images trend inline tendencias">
        <?php foreach ($genre['items'] as $item) : ?>
          <li>
            <a href="<?= search_permalink($item['title']); ?>" title="<?= $item['title']; ?>" class="img" style="background-image:url(<?= photon_resize($item['image'], '96', '96', 2); ?>);"></a>
            <div> <a href="<?= search_permalink($item['title']); ?>" title="<?= $item['title']; ?>" class="name"><?= $item['title']; ?></a> <span><i class="fa fa-clock-o"></i> Date <?= toDayAgo($item['dateRelease']); ?> <i class="fa fa-hdd-o"></i> Genre <?= $item['genre']; ?> </span> </div>
          </li>
        <?php endforeach; ?>
      </ul>
    </section>
